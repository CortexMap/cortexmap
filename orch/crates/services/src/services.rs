...
impl<I: Infra> OrchServices<I> {
    pub fn new(infra: Arc<I>) -> Self {
        let completion_watcher = CompletionWatcher::new(Arc::clone(&infra));
        let region_management = OrchRegionManagement::new(Arc::clone(&infra));
    ...
    I: Infra<Error = E>,
{
    ...
    async fn store_queries(&self, region_id: i32, queries: Vec<String>) -> Result<Vec<Uuid>, Self::Error> {
        self.region_management.store_queries(region_id, queries).await
    }

    async fn generate_queries(&self, region_name: &str, count: u32) -> Result<Vec<String>, Self::Error> {
        self.region_management.generate_queries(region_name, count).await
    }
}
    ...
    I: Infra<Error = E>,
{
    ...
    async fn create_batch(&self, region_id: i32, expected_count: usize) -> Result<Uuid, Self::Error> {
        self.batch_orchestration.create_batch(region_id, expected_count).await
    }

    async fn enqueue_fetch_task(&self, query: String, region_id: i32, priority: i32) -> Result<i64, Self::Error> {
        self.batch_orchestration.enqueue_fetch_task(query, region_id, priority).await
    }

    async fn add_tasks_to_batch(&self, batch_id: Uuid, task_ids: Vec<i64>) -> Result<(), Self::Error> {
        self.batch_orchestration.add_tasks_to_batch(batch_id, task_ids).await
    }
}
