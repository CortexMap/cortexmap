...
async fn test_task_claiming_with_timeout() {
    ...
    let task2 = ctx.infra.get_next_pending_task(1).await.expect("Failed to query again");
    ...
    let task3 = ctx.infra.get_next_pending_task(1).await.expect("Failed to claim after timeout");
...
async fn test_component_status_updates() {
    ...
    let summary_component = components.iter()
...
async fn test_stale_task_reset() {
    ...
    let reset_count = ctx.infra.reset_stale_tasks(1).await.expect("Failed to reset stale tasks");
    assert!(reset_count > 0, "Expected at least one task to be reset");
    
    // Task should now be claimable again
    let claimed = ctx.infra.get_next_pending_task(0).await.expect("Failed to claim reset task")
